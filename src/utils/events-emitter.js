...
class QueueSet {
     ...
     * 获取某一条具体队列
     *
     * @param {string} namespace 需要获取的队列的名称
...
export default class EventsEmitter {
     ...
     * @param {boolean} item.once - 参数，是否只执行一次
     * @return {boolean} - 执行是否成功的结果
     */
    handlerWrapper(type, message, {handler, once}) {
        if (!handler) {
            return false;
        }
